  ...
  server.add(IResolverService_abi, [
    {
      ...
      func: async ([encodedName, data]: Result, request) => {
        ...
        // Convert from compact (r, _vs) to standard (r, s, v) format for OpenZeppelin ECDSA
...
export function makeApp(
  ...
  // Add health check endpoints for Render deployment
  ...
  app.get('/', (_req: any, res: any) => {
    res.status(200).json({
      service: 'ENS Offchain Resolver Gateway',
      status: 'running',
      description: 'CCIP-Read gateway for ENS L2 Subnames',
      endpoints: {
        health: '/health',
        resolver: '/{sender}/{data}.json'
      }
    });
  });
