...
class ReviewsSearchAgent:
    ...
    def search_reviews(self, brand_name: str, sentiment: str = "positive") -> dict:
    ...
    def create_reviews_tool_schema(self):
        ...
        return {
            ...
            "function": {
                "name": "search_reviews",
                "description": "Search for brand reviews with specific sentiment analysis. Use this tool when users want to find positive (or negative) reviews for a particular brand, product, or company. This tool scrapes and analyzes reviews from various sources to provide sentiment-filtered results.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "brand_name": {
...
3. SENTIMENT ANALYSIS: Are they looking for positive reviews.
4. REPUTATION RESEARCH: Do they want to understand customer satisfaction or brand perception?

USE THE TOOL WHEN:
- Asking for positive reviews about a brand/product
- Requesting customer testimonials or feedback
- Wanting to know what customers think about a brand
- Seeking customer satisfaction insights
- Asking about brand reputation from customer perspective
- Looking for success stories or customer experiences
- Requesting sentiment analysis of reviews
...
EXAMPLES:
✅ USE TOOL: "Find positive reviews for Tesla", "What do customers say about Apple products?", "Show me good reviews for Nike shoes", "Customer testimonials for Starbucks"
❌ DON'T USE: "What is Tesla's stock price?", "When was Apple founded?", "Nike company history", "Starbucks locations"

When using the tool, default to searching for positive reviews unless the user specifically asks for negative reviews.
...
2. For each review, you MUST include:
   ...
   - The star rating (e.g., "4 stars", "5 stars")
...
- NEVER EVER talk about negative reviews, filter out and talk ONLY about positive reviews!
...
# Create uAgent
agent = Agent(
    name="brandx_reviews_search_agent",
    port=8080,
    seed="brandx reviews search agent seed",
    mailbox=True,
...
async def startup_handler(ctx: Context):
    ctx.logger.info(f"ASI:One Reviews Search Agent started with address: {ctx.agent.address}")
    ctx.logger.info(f"Reviews endpoint configured: {REVIEWS_MCP_ENDPOINT}")
    ctx.logger.info("Agent is ready to intelligently search for brand reviews using ASI:One!")
    ctx.logger.info("The agent will reason about whether queries need review searches or can be answered directly")
    ctx.logger.info("REST API endpoint available at: http://localhost:8080/reviews/positive")
...
async def handle_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    ctx.logger.info(f"Processing reviews query: {user_query}")
    ...
    try:
        ...
        ctx.logger.info(f"Sent reviews response to {sender}")
        
    except Exception as e:
        error_msg = f"Error processing reviews query: {str(e)}"
...
# REST API Handler for Positive Reviews
@agent.on_rest_post("/reviews/positive", PositiveReviewsRequest, PositiveReviewsResponse)
async def handle_positive_reviews(ctx: Context, req: PositiveReviewsRequest) -> PositiveReviewsResponse:
    ctx.logger.info(f"Received positive reviews request for: {req.brand_name}")
    
    try:
        # Process the positive reviews query using the existing reviews search agent
        reviews_query = f"Find positive reviews for {req.brand_name}"
        response_text = reviews_search_agent.process_reviews_query(reviews_query)
        
        ctx.logger.info(f"Positive reviews search completed for: {req.brand_name}")
        
        return PositiveReviewsResponse(
            success=True,
        ...
        error_msg = f"Error processing positive reviews for {req.brand_name}: {str(e)}"
        ctx.logger.error(error_msg)
...
if __name__ == '__main__':
    print("🚀 Starting ASI:One Reviews Search Agent...")
    print(f"✅ Agent address: {agent.address}")
    print(f"🔗 Reviews endpoint: {REVIEWS_MCP_ENDPOINT}")
    print("📡 Ready to intelligently search for brand reviews")
    print("🧠 Powered by ASI:One AI reasoning and Reviews MCP")
    print("\n🌐 REST API Endpoint:")
    print("POST http://localhost:8080/reviews/positive")
    print("Body: {\"brand_name\": \"Tesla\", \"sentiment\": \"positive\"}")
    print("\n🧪 Test queries (agent will decide whether to search reviews):")
    print("- 'Find positive reviews for Tesla' (will search reviews)")
    print("- 'What do customers say about Apple products?' (will search reviews)")
    print("- 'Show me good reviews for Nike shoes' (will search reviews)")
    print("- 'What is Tesla's stock price?' (will answer directly)")
    print("- 'Customer testimonials for Starbucks' (will search reviews)")
    print("\nPress CTRL+C to stop the agent")
    
    try:
        agent.run()
    except KeyboardInterrupt:
        print("\n🛑 Shutting down ASI:One Reviews Search Agent...")
        print("✅ Agent stopped.")